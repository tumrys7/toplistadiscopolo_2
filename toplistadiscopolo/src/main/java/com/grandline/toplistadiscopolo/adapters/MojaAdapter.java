...
public class MojaAdapter extends BaseAdapter {
    ...
    public MojaAdapter(Activity a, ArrayList<HashMap<String, String>> d) {
        ...
        // Log adapter creation for debugging
        android.util.Log.i("MojaAdapter", "Created adapter with " + cachedSize + " items");
    }
    ...
    public void safeNotifyDataSetChanged() {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            ...
            synchronized(dataLock) {
                android.util.Log.i("MojaAdapter", "safeNotifyDataSetChanged: workingData size = " + 
                    (workingData != null ? workingData.size() : 0) + ", cachedSize = " + cachedSize);
                if (workingData != null) {
            ...
            mainHandler.post(() -> {
                refreshDataFromSource();
                synchronized(dataLock) {
                    android.util.Log.i("MojaAdapter", "safeNotifyDataSetChanged (posted): workingData size = " + 
                        (workingData != null ? workingData.size() : 0) + ", cachedSize = " + cachedSize);
                    if (workingData != null) {
                        notifyDataSetChanged();
                    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        synchronized(dataLock) {
            ...
            if (workingData == null || position < 0 || position >= workingData.size() || position >= cachedSize) {
                ...
                android.util.Log.w("MojaAdapter", "getView called with invalid position " + position + 
                    " (workingData size: " + (workingData != null ? workingData.size() : 0) + 
